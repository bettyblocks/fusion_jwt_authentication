defmodule FusionJWTAuthentication.FusionJWTAuthPlugTest do
  ...
  describe "test fusion auth plug using public certificate" do
    setup do
      Application.put_env(:fusion_jwt_authentication, :token_verifier, TokenCertificateStore)

      start_supervised(FusionJWTAuthentication.CertificateStore)

      on_exit(fn ->
        Application.delete_env(:fusion_jwt_authentication, :TokenCertificateStore)
      end)
    end
  ...
  describe "fusion jwt auth plug using JWKS endpoint" do
    setup do
      Application.put_env(:fusion_jwt_authentication, :token_verifier, Token)
    ...
    test "validate token using jwks and set the cas token", %{cas_token: cas_token} do
      ...
      jwt = Token.generate_and_sign!(claims, TestUtils.create_signer_with_kid("id2"))
    ...
    test "if kid does not match, returns 401", %{cas_token: cas_token} do
      ...
      jwt = Token.generate_and_sign!(claims, TestUtils.create_signer_with_kid("id4"))
